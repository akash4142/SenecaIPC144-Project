...
void addPatient(struct Patient patient[], int max) 
{
    ...
    if (Room != 0)
    {
        ...
        patient[freeIdx].patientNumber = nextPatientNumber(patient, max);
        inputPatient(&patient[freeIdx]); 
        printf("\n");
...
void editPatient(struct Patient patient[], int max)
{
    ...
    scanf("%d", &intInput);
    clearInputBuffer();
    Found = findPatientIndexByPatientNum(intInput, patient, max); // 3 

    if (Found != -1) // -1 is not found
    {
        printf("\n");
...
void removePatient(struct Patient patient[], int max)
{
    ...
    scanf("%d", &intInput);
    clearInputBuffer();
    Found = findPatientIndexByPatientNum(intInput, patient, max); 
...
int nextPatientNumber(const struct Patient patient[], int max)
{
    ...
    // find max
    Max = patient[0].patientNumber; 
    for (i = 0; i < max; i++)
    {
        if (Max < patient[i].patientNumber)
        {
            Max = patient[i].patientNumber;
        }
    }
